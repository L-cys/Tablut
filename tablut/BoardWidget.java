...
class BoardWidget extends Pad {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (Square ad : ADJACENT_THRONE) {
            g.fillRect(cx(ad) + OFFSET, cy(ad) + OFFSET,
        ...
        for (int i = 0; i < SIZE; i = i + 1) {
            ...
            String number = Integer.toString(SIZE - i);
            String cha = Character.toString(alp.charAt(i));
            g.drawString(number, OFFSET, MARGIN + i * SQUARE_SIDE);
            g.drawString(cha,
                    i * SQUARE_SIDE + SQUARE_SIDE, SIZE * SQUARE_SIDE + MARGIN);
        }
    ...
    private void drawPiece(Graphics2D g, Square s) {
        if (_board.get(s) == KING) {
            ...
            g.setColor(Color.red);
            g.setFont(KING_FONT);
            g.drawString("K", cx(s) + 4 * OFFSET, cy(s) + 10 * OFFSET);
        }
    ...
    private void click(Square s) {
        if (!_hasclick) {
            if (_board.turn() == _board.get(s).side()) {
                _hasclick = true;
                _clicked = s;
            }
        } else {
            if (_board.get(s) == EMPTY) {
    ...
    /**
     * True is there is a square has been clicked.
     */
    private boolean _hasclick;

    /**
     * A square to store previous click square.
     */
